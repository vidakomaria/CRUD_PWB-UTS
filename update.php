...
if (isset($_POST["Name"]) && isset($_POST["CountryCode"]) && isset($_POST["District"]) && isset($_POST["Population"])) {
    ...
    if (strlen($nama) == 0) {
        ...
        $data = "UPDATE city SET Name=:Name, CountryCode=:CountryCode, District=:District, Population=:Population WHERE ID=:id";
        $statement = $connection->prepare($data);
        if ($statement->execute([":id" => $get_id, ":Name" => $nama, ":CountryCode" => $kode, ":District" => $distrik, ":Population" => $populasi])) {
...
<div class="container">
    <div class="card">
        <div class="card-header">
            <h2><a href="index.php" class="btn btn-primary">BACK</a> UPDATE</h2>
        </div>
